...
if [ -z "${PR_ID}" ]; then
  ...
  if HEAD_COMMIT="$( git rev-parse HEAD )"; then
    if SEARCH_RESULTS="$( curl -s ${GH_API_URL}/search/issues?q=sha:${HEAD_COMMIT}+type:pr+repo:AimonaStudio/rrreol )"; then
...
if [ -z "${PR_ID}" ]; then
 ...
 PATCH=$( curl -sL https://github.com/AimonaStudio/rrreol/pull/${PR_ID}.patch | grep '^From ' )
